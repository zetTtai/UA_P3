...
public class PlayerMovementScore_PreP4Test {
	...
	public void testCompareTo() {
		try {
			...
			assertTrue(scPeter.compareTo(scLaura)==0);
			scLaura.score(initialPos.above());
			//Ahora laura tiene más score--> Invoca ella y devuelve positivo
			assertTrue(scLaura.compareTo(scPeter) > 0);
			scPeter.score(new Location(world, 0, 90,0));
			assertTrue(scLaura.compareTo(scPeter) < 0);//Laura es menor, devuelve negativo
	...
	public void testScoreLocation() {
		try {
			...
			scPeter.score(initialPos.above());
			assertEquals(1.0,scPeter.getScoring(),0.01);
	
			scPeter.score(initialPos.above().above());
			assertEquals(2.0,scPeter.getScoring(),0.01);
			
			scPeter.score(new Location(world, 0,90,0));
			assertEquals(24.0,scPeter.getScoring(),0.01);
	...
	public void testToString() {
		try {
			...
			scPeter.score(initialPos.above().above());//LeL
