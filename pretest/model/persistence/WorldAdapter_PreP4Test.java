...
public class WorldAdapter_PreP4Test {
	...
	public void setUp() throws Exception {
		world10 =  new World(5, 10,"World 10x10","Joel");//Límites: 
	...
	public void testGetMapBlock1() {
		...
		for (Location relative : setLoc) {
			...
			try {
				...
				if(Location.check(absolute) && block == null)//Está dentro de los límites y es null
					block= blockAIR;//Para poder comprobar con el aire
				
				Block blockFromMap= mapBlock.get(relative);
				assertEquals(block, blockFromMap);
			} catch (Exception e) {
	...
	public void testGetCreatures1() {
		...
		for(Creature c: listCreature) {
			if(!okLocation(c.getLocation(), base)) 
				fail("La location: " + c.getLocation() + " no se encuentra entre [0,15] para x,y,z");
			
			try {
				if(world75.getCreatureAt(c.getLocation()) == null) {//No existe criatura en esa posición
					fail("En la location: " + c.getLocation() + " no se encuentra ninguna criatura");
				}
				if(world75.getCreatureAt(c.getLocation()).getSymbol() == 'L')
					nAnimals++;
				if(world75.getCreatureAt(c.getLocation()).getSymbol() == 'M')
					nMonsters++;
			} catch (BadLocationException e) {e.printStackTrace();}
		}
		assertEquals(nAnimals, 3);
		assertEquals(nMonsters, 12);
	}
	...
	public void testGetCreatures2() {
		...
		for(Creature c: listCreature) {
			...
			try {
				if(world10.getCreatureAt(c.getLocation()) == null) {//No existe criatura en esa posición
					fail("En la location: " + c.getLocation() + " no se encuentra ninguna criatura");
				}
				if(world10.getCreatureAt(c.getLocation()).getSymbol() == 'L')
					nAnimals++;
				if(world10.getCreatureAt(c.getLocation()).getSymbol() == 'M')
					nMonsters++;
			} catch (BadLocationException e) {e.printStackTrace();}
		}
		assertEquals(nAnimals, 4);
		assertEquals(nMonsters, 3);
	}
...
public void testGetItems1() {
		...
		Set<Location> setLoc = mapItems.keySet();//Importante(Key --> Location)
		
		Iterator it= setLoc.iterator();//Puntero para recorrer Map
		
		while(it.hasNext()) {
			Location key= (Location)it.next();//Downcasting a Location
			//System.out.println("KEY--> " + key + " || " + "VALUE--> " + mapItems.get(key));
			if(!okLocation(key, base)) 
				fail("La location: " + key + " no se encuentra entre [0,15] para x,y,z");
		}
 ...
 *
 *  Observación: Todas las posiciones NO están dentro de los límites de world10
 */
//TODO
@Test
public void testGetItems2() {
		...
		Iterator it= setLoc.iterator();//Puntero para recorrer [Map mapItems.keySet().iterator();]
